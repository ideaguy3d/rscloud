...
!function (e, t, n) {
  ...
  function o(e, o, i, r, l, a, s, p, h, f, g, $) {
    function x() {
      i.initOptionalProperties(e, h, {
        searchText: "",
        selectedItem: null,
        clearButton: !1
      }), l(o), A(), i.nextTick(function () {
    ...
    function v() {
      ...
      function n() {
        var e = ye.scrollContainer.getBoundingClientRect(),
            t = {};
        e.right > p.right - d && (t.left = a.right - e.width + "px"), ye.$.scrollContainer.css(t);
      }

      if (!ye) return i.nextTick(v, !1, e);
      var r,
          ...
          x = a.width,
          C = t(),
          b = e.dropdownPosition;
      if (b || (b = f > g && p.height - f - d < l ? "top" : "bottom"), h.mdFloatingLabel && ($ += m, x -= 2 * m), r = {
        left: $ + "px",
        minWidth: x + "px",
        maxWidth: Math.max(a.right - p.left, p.right - a.left) - d + "px"
      }, "top" === b) r.top = "auto", r.bottom = g + "px", r.maxHeight = Math.min(l, a.top - p.top - d) + "px";else {
        var w = p.bottom - a.bottom - d + i.getViewportTop();
        r.top = f - C + "px", r.bottom = "auto", r.maxHeight = Math.min(l, w) + "px";
      }
    ...
    function A() {
      ...
      }), e.$watch("searchText", n ? i.debounce(H, n) : H), e.$watch("selectedItem", O), t.element(a).on("resize", Se), e.$on("$destroy", T);
    }

    function T() {
      if (we.hidden || i.enableScrolling(), t.element(a).off("resize", Se), ye) {
    ...
    function I() {
      ...
      ye = {
        ...
        snap: e.snap,
        root: document.body
      }, ye.li = ye.ul.getElementsByTagName("li"), ye.$ = M(ye), De = ye.$.input.controller("ngModel");
    }

    function E() {
      var e, n;

      for (e = o; e.length && (n = e.attr("md-autocomplete-snap"), !t.isDefined(n)); e = e.parent());

      if (e.length) return {
        snap: e[0],
        wrap: "width" === n.toLowerCase() ? e[0] : o.find("md-autocomplete-wrap")[0]
      };
      var i = o.find("md-autocomplete-wrap")[0];
      return {
        snap: i,
        wrap: i
      };
    ...
    function R(e, n) {
      !e && n ? (v(), se(!0, Oe.Count | Oe.Selected), ye && (i.disableScrollAround(ye.ul), ke = L(t.element(ye.wrap)))) : e && !n && (i.enableScrolling(), ke && (ke(), ke = null));
    }
    ...
    function D() {
      Re || we.hidden || ye.input.focus(), Ee = !1, we.hidden = J();
    }
    ...
    function O(n, o) {
      C(), n ? U(n).then(function (t) {
        e.searchText = t, F(n, o);
      }) : o && e.searchText && U(o).then(function (n) {
    ...
    function H(t, n) {
      we.index = Y(), t !== n && (C(), U(e.selectedItem).then(function (o) {
        t !== o && (e.selectedItem = null, t !== n && B(), re() ? xe() : (we.matches = [], G(!1), se(!1, Oe.Count)));
      }));
    }

    function V(e) {
      Re = !1, Ee || (we.hidden = J(), be("ngBlur", {
        $event: e
      }));
    }

    function j(e) {
      e && (Ee = !1, Re = !1), ye.input.blur();
    }

    function z(e) {
      Re = !0, Q() && re() && xe(), we.hidden = J(), be("ngFocus", {
        $event: e
      });
    }
    ...
    function W(t) {
      switch (t.keyCode) {
        case r.KEY_CODE.DOWN_ARROW:
          if (we.loading) return;
          t.stopPropagation(), t.preventDefault(), we.index = Math.min(we.index + 1, we.matches.length - 1), he(), se(!1, Oe.Selected);
          break;

        case r.KEY_CODE.UP_ARROW:
          if (we.loading) return;
          t.stopPropagation(), t.preventDefault(), we.index = we.index < 0 ? we.matches.length - 1 : Math.max(0, we.index - 1), he(), se(!1, Oe.Selected);
          break;

        case r.KEY_CODE.TAB:
          if (D(), we.hidden || we.loading || we.index < 0 || we.matches.length < 1) return;
          ae(we.index);
          break;

        case r.KEY_CODE.ENTER:
          if (we.hidden || we.loading || we.index < 0 || we.matches.length < 1) return;
          if (ne()) return;
          t.stopPropagation(), t.preventDefault(), ae(we.index);
          break;

        case r.KEY_CODE.ESCAPE:
          if (t.preventDefault(), !X()) return;
          t.stopPropagation(), ue(), e.searchText && Z("clear") && de(), we.hidden = !0, Z("blur") && j(!0);
      }
    }
    ...
    function K(e) {
      if (!e) return n;
      var t = {};
      return we.itemName && (t[we.itemName] = e), t;
    }
    ...
    function ae(t) {
      i.nextTick(function () {
        U(we.matches[t]).then(function (e) {
          var t = ye.$.input.controller("ngModel");
    ...
    function de() {
      ...
      t.initCustomEvent("change", !0, !0, {
        value: ""
      }), ye.input.dispatchEvent(t), ye.input.blur(), e.searchText = "", ye.input.focus();
    }
    ...
    function se(e, t) {
      var n = e ? "polite" : "assertive",
          o = [];
      t & Oe.Selected && we.index !== -1 && o.push(ie()), t & Oe.Count && o.push(f.resolve(pe())), f.all(o).then(function (e) {
        $.announce(e.join(" "), n);
      });
    }

    function pe() {
      switch (we.matches.length) {
        case 0:
    ...
    function $e() {
      var e = (we.scope.searchText || "").length;
      return we.hasNotFound && !te() && (!we.loading || fe()) && e >= _() && (Re || Ee) && !ne();
    }

    function xe() {
      var t = e.searchText || "",
          n = t.toLowerCase();
      !e.noCache && Ie[n] ? Ce(Ie[n]) : me(t), we.hidden = J();
    }

    function Ce(t) {
      we.matches = t, we.hidden = J(), we.loading && G(!1), e.selectOnMatch && ve(), v(), se(!0, Oe.Count);
    }
    ...
    function ve() {
      var t = e.searchText,
          n = we.matches,
          o = n[0];
      1 === n.length && U(o).then(function (n) {
        var o = t == n;
        e.matchInsensitive && !o && (o = t.toLowerCase() == n.toLowerCase()), o && ae(0);
      });
    }
    ...
    function n(e, n, o) {
      return function (e, n, i) {
        ...
        var a = e.$mdAutocompleteCtrl,
            c = a.parent.$new(),
            u = a.itemName;
        r("$index", "$index"), r("item", u), l(), o(c, function (e) {
          n.after(e);
        });
      };
    }
  ...
  t.module("material.components.autocomplete", ["material.core", "material.components.icon", "material.components.virtualRepeat"]), o.$inject = ["$scope", "$element", "$mdUtil", "$mdConstant", "$mdTheming", "$window", "$animate", "$rootElement", "$attrs", "$q", "$log", "$mdLiveAnnouncer"], t.module("material.components.autocomplete").controller("MdAutocompleteCtrl", o);
  var c = 48,
      u = 5,
      d = 8,
      m = 2;
