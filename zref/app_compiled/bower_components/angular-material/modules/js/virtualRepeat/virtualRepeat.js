...
(function (window, angular, undefined) {
  ...
  function VirtualRepeatContainerController($$rAF, $mdUtil, $mdConstant, $parse, $rootScope, $window, $scope, $element, $attrs) {
    ...
    this.offsetter = this.scroller.querySelector('.md-virtual-repeat-offsetter'); // After the dom stablizes, measure the initial size of the container and
    ...
    $$rAF(angular.bind(this, function () {
      ...
      var jWindow = angular.element($window); // Make one more attempt to get the size if it is 0.
  ...
  VirtualRepeatContainerController.prototype.updateSize = function () {
    ...
    this.size = this.isHorizontal() ? this.$element[0].clientWidth : this.$element[0].clientHeight; // Recheck the scroll position after updating the size. This resolves
  ...
  VirtualRepeatContainerController.prototype.sizeScroller_ = function (size) {
    var dimension = this.getDimensionName_();
    var crossDimension = this.isHorizontal() ? 'height' : 'width'; // Clear any existing dimensions.

    this.sizer.innerHTML = ''; // If the size falls within the browser's maximum explicit size for a single element, we can
    ...
    if (size < this.maxElementPixels) {
      ...
      this.sizer.style[crossDimension] = 'auto'; // Divide the total size we have to render into N max-size pieces.

      var numChildren = Math.floor(size / this.maxElementPixels); // Element template to clone for each max-size piece.
  ...
  VirtualRepeatContainerController.prototype.autoShrink_ = function (size) {
    ...
    if (this.autoShrink && shrinkSize !== this.size) {
      ...
      if (!currentSize || shrinkSize < currentSize) {
        ...
        this.originalSize = null; // We determine the repeaters size again, if the original size was zero.
        // The originalSize needs to be null, to be able to determine the size.

        if (!_originalSize) this.updateSize(); // Apply the original size or the determined size back to the container, because
  ...
  function VirtualRepeatController($scope, $element, $attrs, $browser, $document, $rootScope, $$rAF, $mdUtil) {
    ...
    this.endIndex = 0; // TODO: measure width/height of first element from dom if not provided.
    // getComputedStyle?
  ...
  VirtualRepeatController.prototype.virtualRepeatUpdate_ = function (items, oldItems) {
    ...
    var lengthChanged = false; // If the number of items shrank, keep the scroll position.
    ...
    var i,
        block,
        newStartBlocks = [],
        newEndBlocks = []; // Collect blocks at the top.
  ...
  VirtualRepeatController.prototype.updateBlock_ = function (block, index) {
    ...
    block.new = false; // Update and digest the block's scope.

    this.updateScope_(block.scope, index); // Perform digest before reattaching the block.
