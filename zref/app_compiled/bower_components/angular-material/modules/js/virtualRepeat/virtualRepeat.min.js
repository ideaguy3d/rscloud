...
!function (t, e, i) {
  ...
  function o(t, i, s, n, o, r, h, l, a) {
    ...
    }))) : this.topIndex = 0, this.scroller = l[0].querySelector(".md-virtual-repeat-scroller"), this.sizer = this.scroller.querySelector(".md-virtual-repeat-sizer"), this.offsetter = this.scroller.querySelector(".md-virtual-repeat-offsetter");
    var c = e.bind(this, this.updateSize);
    t(e.bind(this, function () {
      c();
      var t = i.debounce(c, 10, null, !1),
          s = e.element(r);
      this.size || t(), s.on("resize", t), h.$on("$destroy", function () {
        s.off("resize", t);
    ...
    return {
      ...
      compile: function (e, i) {
        var s = i.mdVirtualRepeat,
            ...
            o = n[1],
            r = t(n[2]),
            h = i.mdExtraName && t(i.mdExtraName);
  ...
  o.prototype.register = function (t) {
    ...
    var e = this.getDimensionName_(),
        i = this.isHorizontal() ? "height" : "width";
    if (this.sizer.innerHTML = "", t < this.maxElementPixels) this.sizer.style[e] = t + "px";else {
      this.sizer.style[e] = "auto", this.sizer.style[i] = "auto";
      var s = Math.floor(t / this.maxElementPixels),
          n = document.createElement("div");
      n.style[e] = this.maxElementPixels + "px", n.style[i] = "1px";
    ...
    t || this.maxSize || (this.scroller.scrollLeft = this.scrollSize, this.maxSize = this.scroller.scrollLeft);
    var e = this.isHorizontal() ? t ? this.scroller.scrollLeft : this.maxSize - this.scroller.scrollLeft : this.scroller.scrollTop;
    ...
    if (!(e === this.scrollOffset || e > this.scrollSize - this.size)) {
      ...
      if (i) {
        var s = Math.max(0, Math.floor(e / i) - a),
            n = (this.isHorizontal() ? "translateX(" : "translateY(") + (!this.isHorizontal() || t ? s * i : -(s * i)) + "px)";

        if (this.scrollOffset = e, this.offsetter.style.webkitTransform = n, this.offsetter.style.transform = n, this.bindTopIndex) {
    ...
    s !== this.itemsLength && (n = !0, this.itemsLength = s), this.items = t, (t !== i || n) && this.updateIndexes_(), this.parentNode = this.$element[0].parentNode, n && this.container.setScrollSize(s * this.itemSize);
    var r,
        h = !1;
    this.isFirstRender && (h = !0, this.isFirstRender = !1, r = this.$attrs.mdStartIndex ? this.$scope.$eval(this.$attrs.mdStartIndex) : this.container.topIndex, this.container.scrollToIndex(r)), Object.keys(this.blocks).forEach(function (t) {
      var e = parseInt(t, 10);
    ...
    for (l = this.newStartIndex; l < this.newEndIndex && null == this.blocks[l]; l++) a = this.getBlock_(l), this.updateBlock_(a, l), c.push(a);

    for (; null != this.blocks[l]; l++) this.updateBlock_(this.blocks[l], l);

    for (var p = l - 1; l < this.newEndIndex; l++) a = this.getBlock_(l), this.updateBlock_(a, l), d.push(a);

    c.length && this.parentNode.insertBefore(this.domFragmentFromBlocks_(c), this.$element[0].nextSibling), d.length && this.parentNode.insertBefore(this.domFragmentFromBlocks_(d), this.blocks[p] && this.blocks[p].element[0].nextSibling), h && this.container.scrollToIndex(r), this.$browser.$$checkUrlChange = this.browserCheckUrlChange, this.startIndex = this.newStartIndex, this.endIndex = this.newEndIndex, this.isVirtualRepeatUpdating_ = !1;
    ...
    return this.transclude(e.bind(this, function (e, s) {
      i = {
        ...
        "new": !0,
        scope: s
      }, this.updateScope_(s, t), this.parentNode.appendChild(e[0]);
    })), i;
    ...
    var t = this.items ? this.items.length : 0,
        e = Math.ceil(this.container.getSize() / this.itemSize);
    this.newStartIndex = Math.max(0, Math.min(t - e, Math.floor(this.container.getScrollOffset() / this.itemSize))), this.newVisibleEnd = this.newStartIndex + e + a, this.newEndIndex = Math.min(t, this.newVisibleEnd), this.newStartIndex = Math.max(0, this.newStartIndex - a);
