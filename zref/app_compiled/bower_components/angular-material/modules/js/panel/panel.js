...
(function (window, angular, undefined) {
  ...
  function definePreset(name, preset) {
    if (!name || !preset) {
      throw new Error('mdPanelProvider: The panel preset definition is ' + 'malformed. The name and preset object are required.');
  ...
  MdPanelService.prototype.create = function (preset, config) {
    ...
    preset = preset || {};
    config = config || {}; // If the passed-in config contains an ID and the ID is within _trackedPanels,
    ...
    }, this._defaultConfigOptions, config, preset); // Create the panelRef and add it to the `_trackedPanels` object.

    var panelRef = new MdPanelRef(this._config, this._$injector);
    this._trackedPanels[config.id] = panelRef; // Add the panel to each of its requested groups.
  ...
  MdPanelService.prototype._getPresetByName = function (preset) {
    if (!this._presets[preset]) {
      throw new Error('mdPanel: The panel preset configuration that you ' + 'requested does not exist. Use the $mdPanelProvider to create a ' + 'preset before requesting one.');
    }
  ...
  MdPanelService.prototype._wrapTemplate = function (origTemplate) {
    var template = origTemplate || ''; // The panel should be initially rendered offscreen so we can calculate
  ...
  function MdPanelRef(config, $injector) {
    ...
    this._$$rAF = $injector.get('$$rAF'); // Public variables.
    ...
    this.isAttached = false; // Private variables.
  ...
  MdPanelRef.prototype.detach = function () {
    ...
    var detachFn = function () {
      self._removeEventListeners(); // Remove the focus traps that we added earlier for keeping focus within
      // the panel.
  ...
  MdPanelRef.prototype._compile = function () {
    var self = this; // Compile the element via $mdCompiler. Note that when using a
    ...
    return self._$mdCompiler.compile(self.config).then(function (compileData) {
      ...
      if (config.contentElement) {
        var panelEl = compileData.element; // Since mdPanel modifies the inline styles and CSS classes, we need
      ...
      self._compilerCleanup = compileData.cleanup; // Attach the panel to the proper place in the DOM.
  ...
  MdPanelRef.prototype._addStyles = function () {
    ...
    return this._$q(function (resolve) {
      ...
      var hideAndResolve = function () {
        ...
        self._setTheming(); // Remove offscreen class and add hidden class.
      ...
      self._$rootScope['$$postDigest'](function () {
        ...
        self._updatePosition(true); // Theme the element and container.
  ...
  MdPanelRef.prototype._updatePosition = function (init) {
    ...
    if (positionConfig) {
      positionConfig._setPanelPosition(this.panelEl); // Hide the panel now that position is known.
  ...
  MdPanelRef.prototype._configureEscapeToClose = function () {
    if (this.config['escapeToClose']) {
      ...
      parentTarget.on('keydown', keyHandlerFn); // Queue remove listeners function
  ...
  MdPanelRef.prototype._configureClickOutsideToClose = function () {
    if (this.config['clickOutsideToClose']) {
      ...
      target.on('mouseup', mouseupHandler); // Queue remove listeners function
  ...
  */
  ...
  MdPanelRef.prototype._configureScrollListener = function () {
    ...
    if (!this.config['disableParentScroll']) {
      ...
      this._$window.addEventListener('scroll', onScroll, true); // Queue remove listeners function.
  ...
  MdPanelRef.prototype._configureTrapFocus = function () {
    ...
    if (this.config['trapFocus']) {
      var element = this.panelEl; // Set up elements before and after the panel to capture focus and
      // redirect back into the panel.

      this._topFocusTrap = FOCUS_TRAP_TEMPLATE.clone()[0];
      this._bottomFocusTrap = FOCUS_TRAP_TEMPLATE.clone()[0]; // When focus is about to move out of the panel, we want to intercept it
      ...
      this._bottomFocusTrap.addEventListener('focus', focusHandler); // Queue remove listeners function
      ...
      }, this)); // The top focus trap inserted immediately before the md-panel element (as
  ...
  */
  ...
  MdPanelPosition.prototype.getTransform = function () {
    ...
    var translateY = this._reduceTranslateValues('translateY', this._translateY); // It's important to trim the result, because the browser will ignore the set
  ...
  MdPanelPosition.prototype._setPanelPosition = function (panelEl) {
    ...
    panelEl.removeClass('_md-panel-position-adjusted'); // Only calculate the position if necessary.
  ...
  MdPanelAnimation.prototype.animateOpen = function (panelEl) {
    ...
    var animationOptions = {}; // Include the panel transformations when calculating the animations.
  ...
  MdPanelAnimation.prototype.animateClose = function (panelEl) {
    var animator = this._$mdUtil.dom.animator;
    var reverseAnimationOptions = {}; // Include the panel transformations when calculating the animations.
