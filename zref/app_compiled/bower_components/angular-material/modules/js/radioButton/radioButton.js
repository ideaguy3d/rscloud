...
(function (window, angular, undefined) {
  ...
  function mdRadioGroupDirective($mdUtil, $mdConstant, $mdTheming, $timeout) {
    ...
    function linkRadioGroup(scope, element, attr, ctrls) {
      ...
      function keydownListener(ev) {
        var keyCode = ev.which || ev.keyCode; // Only listen to events that we originated ourselves
    ...
    function createRadioGroupControllerProto() {
      return {
        ...
        setViewValue: function (value, eventType) {
          this._ngModelCtrl.$setViewValue(value, eventType); // update the other radio buttons as well
    ...
    function changeSelectedButton(parent, increment) {
      ...
      if (buttons.count()) {
        ...
        var target = buttons[increment < 0 ? 'previous' : 'next'](selected, validate) || buttons.first(); // Activate radioButton's click listener (triggerHandler won't create a real click event)
  ...
  function mdRadioButtonDirective($mdAria, $mdUtil, $mdTheming) {
    ...
    function link(scope, element, attr, rgCtrl) {
      var lastChecked;
      $mdTheming(element);
      configureAria(element, scope); // ngAria overwrites the aria-checked inside a $watch for ngValue.
