...
(function (window, angular, undefined) {
  ...
  function SidenavService($mdComponentRegistry, $mdUtil, $q, $log) {
    ...
    function findInstance(handle, shouldWait) {
      ...
      if (!instance && !shouldWait) {
        ...
        $log.error($mdUtil.supplant(errorMsg, [handle || ""])); // The component has not registered itself... most like NOT yet created
  ...
  function SidenavFocusDirective() {
    return {
      restrict: 'A',
      require: '^mdSidenav',
      link: function (scope, element, attr, sidenavCtrl) {// @see $mdUtil.findFocusTarget(...)
      }
    };
  }
  /**
  ...
  * The $mdMedia() service is exposed to the is-locked-open attribute, which
    ...
    function postLink(scope, element, attr, sidenavCtrl) {
      ...
      if (attr.mdDisableScrollTarget) {
        ...
        if (disableScrollTarget) {
          ...
          $log.warn($mdUtil.supplant('mdSidenav: couldn\'t find element matching ' + 'selector "{selector}". Falling back to parent.', {
      ...
      $mdTheming(element); // The backdrop should inherit the sidenavs theme,
      ...
      scope.$watch('isOpen', updateIsOpen); // Publish special accessor for the Controller instance
  ...
  function SidenavController($scope, $attrs, $mdComponentRegistry, $q, $interpolate) {
    var self = this; // Use Default internal method until overridden by directive postLink
    // Synchronous getters
    ...
    var componentId = hasDataBinding ? $interpolate(rawId)($scope.$parent) : rawId; // Register the component.

    self.destroy = $mdComponentRegistry.register(self, componentId); // Watch and update the component, if the id has changed.
