...
(function (window, angular, undefined) {
  ...
  function MdSwitch(mdCheckboxDirective, $mdUtil, $mdConstant, $parse, $$rAF, $mdGesture, $timeout) {
    ...
    function mdSwitchCompile(element, attr) {
      var checkboxLink = checkboxDirective.compile(element, attr).post; // No transition on initial load.
      ...
      return function (scope, element, attr, ctrls) {
        ...
        var labelContainer = angular.element(element[0].querySelector('.md-label')); // no transition on initial load
        ...
        attr.$observe('mdInvert', function (newValue) {
          ...
          isInverted ? element.prepend(labelContainer) : element.prepend(switchContainer); // Toggle a CSS class to update the margin.
        ...
        function onDrag(ev) {
          ...
          var percent = ev.pointer.distanceX / drag.width; //if checked, start from right. else, start from left

          var translate = ngModel.$viewValue ? 1 + percent : percent; // Make sure the switch stays inside its bounds, 0-1%
        ...
        function onDragEnd(ev) {
          ...
          ev.stopPropagation();
          element.removeClass('md-dragging');
          thumbContainer.css($mdConstant.CSS.TRANSFORM, ''); // We changed if there is no distance (this is a click a click),
          ...
          drag = null; // Wait for incoming mouse click
