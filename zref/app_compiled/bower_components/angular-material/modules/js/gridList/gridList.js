...
(function (window, angular, undefined) {
  ...
  function GridListDirective($interpolate, $mdConstant, $mdGridLayout, $mdMedia) {
    ...
    function postLink(scope, element, attrs, ctrl) {
      ...
      element.attr('role', 'list'); // Provide the controller with a way to trigger layouts.
      ...
      function layoutDelegate(tilesInvalidated) {
        ...
        }).reflow().performance(); // Report layout
      ...
      var endSymbol = $interpolate.endSymbol(); // Returns an expression wrapped in the interpolator's start and end symbols.
      ...
      var UNIT = $interpolate(expr('share') + '% - (' + expr('gutter') + ' * ' + expr('gutterShare') + ')'); // The horizontal or vertical position of a tile, e.g., the 'top' or 'left' property value.
      // The position comes the size of a 1x1 tile plus gutter for each previous tile in the
      // row/column (offset).

      var POSITION = $interpolate('calc((' + expr('unit') + ' + ' + expr('gutter') + ') * ' + expr('offset') + ')'); // The actual size of a tile, e.g., width or height, taking rowSpan or colSpan into account.
      ...
      function getTileStyle(position, spans, colCount, rowCount, gutter, rowMode, rowHeight) {
        ...
        var hShare = 1 / colCount * 100; // Fraction of the gutter size that each column takes up.

        var hGutterShare = (colCount - 1) / colCount; // Base horizontal size of a column.
        ...
        switch (rowMode) {
          ...
          case 'ratio':
            ...
            var vShare = hShare / rowHeight; // Base veritcal size of a row.
          ...
          case 'fit':
            ...
            var vGutterShare = (rowCount - 1) / rowCount; // Percent of the available vertical space that one row takes up.

            var vShare = 1 / rowCount * 100; // Base vertical size of a row.
  ...
  function GridLayoutFactory($mdUtil) {
    ...
    function calculateGridFor(colCount, tileSpans) {
      ...
      function reserveSpace(spans, i) {
        ...
        var start = 0,
            end = 0; // TODO(shyndman): This loop isn't strictly necessary if you can
  ...
  function GridTileDirective($mdMedia) {
    ...
    function postLink(scope, element, attrs, gridCtrl) {
      // Apply semantics
      element.attr('role', 'listitem'); // If our colspan or rowspan changes, trigger a layout

      var unwatchAttrs = $mdMedia.watchResponsiveAttributes(['md-colspan', 'md-rowspan'], attrs, angular.bind(gridCtrl, gridCtrl.invalidateLayout)); // Tile registration/deregistration
