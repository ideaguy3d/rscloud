...
(function (window, angular, undefined) {
  ...
  function MdChip($mdTheming, $mdUtil, $compile, $timeout) {
    ...
    function postLink(scope, element, attr, ctrls) {
      ...
      if (chipsController) {
        chipController.init(chipsController); // Append our delete hint to the div.md-chip-content (which does not exist at compile time)

        chipContentElement.append($compile(deleteHintTemplate)(scope)); // When a chip is blurred, make sure to unset (or reset) the selected chip so that tabbing
  ...
  MdChipsCtrl.prototype.init = function () {
    var ctrl = this; // Set the wrapper ID

    ctrl.wrapperId = '_md-chips-wrapper-' + ctrl.$mdUtil.nextUid(); // Setup a watcher which manages the role and aria-owns attributes
  ...
  MdChipsCtrl.prototype.setupInputAria = function () {
    var input = this.$element.find('input'); // If we have no input, just return
  ...
  MdChipsCtrl.prototype.setupWrapperAria = function () {
    ...
    if (this.items && this.items.length) {
      ...
      // no items.
      wrapper.attr('role', 'listbox'); // Generate some random (but unique) IDs for each chip
  ...
  MdChipsCtrl.prototype.inputKeydown = function (event) {
    var chipBuffer = this.getChipBuffer(); // If we have an autocomplete, and it handled the event, we have nothing to do
    ...
    if (this.separatorKeys.indexOf(event.keyCode) !== -1) {
      ...
      event.preventDefault(); // Only append the chip and reset the chip buffer if the max chips limit isn't reached.
  ...
  MdChipsCtrl.prototype.chipKeydown = function (event) {
    ...
    switch (event.keyCode) {
      ...
      case this.$mdConstant.KEY_CODE.DELETE:
        ...
        event.preventDefault(); // Cancel the delete action only after the event cancel. Otherwise the page will go back.
      ...
      case this.$mdConstant.KEY_CODE.LEFT_ARROW:
        event.preventDefault(); // By default, allow selection of -1 which will focus the input; if we're readonly, don't go
        // below 0
  ...
  MdChipsCtrl.prototype.removeAndSelectAdjacentChip = function (index) {
    ...
    self.removeChip(index); // The dobule-timeout is currently necessary to ensure that the DOM has finalized and the select()
  ...
  MdChipsCtrl.prototype.appendChip = function (newChip) {
    ...
    if (newChip == null || this.items.indexOf(newChip) + 1) return; // Append the new chip onto our list

    var length = this.items.push(newChip);
    var index = length - 1; // Update model validation

    this.ngModelCtrl.$setDirty();
    this.validateModel(); // If they provide the md-on-add attribute, notify them of the chip addition
  ...
  MdChipsCtrl.prototype.getChipBuffer = function () {
    var chipBuffer = !this.userInputElement ? this.chipBuffer : this.userInputNgModelCtrl ? this.userInputNgModelCtrl.$viewValue : this.userInputElement[0].value; // Ensure that the chip buffer is always a string. For example, the input element buffer might be falsy.
  ...
  MdChipsCtrl.prototype.removeChip = function (index) {
    var removed = this.items.splice(index, 1); // Update model validation
  ...
  MdChipsCtrl.prototype.selectChip = function (index) {
    if (index >= -1 && index <= this.items.length) {
      this.selectedChip = index; // Fire the onSelect if provided
  ...
  MdChipsCtrl.prototype.onInputFocus = function () {
    this.inputHasFocus = true; // Make sure we have the appropriate ARIA attributes

    this.setupInputAria(); // Make sure we don't have any chips selected
  ...
  MdChipsCtrl.prototype.configureUserInput = function (inputElement) {
    this.userInputElement = inputElement; // Find the NgModelCtrl for the input element

    var ngModelCtrl = inputElement.controller('ngModel'); // `.controller` will look in the parent as well.
    ...
    var ctrl = this; // Run all of the events using evalAsync because a focus may fire a blur in the same digest loop
  ...
  function MdChips($mdTheming, $mdUtil, $compile, $log, $timeout, $$mdSvgRegistry) {
    ...
    function compile(element, attr) {
      ...
      }).join(','); // Set the chip remove, chip contents and chip input templates. The link function will put
      ...
      var chipRemoveTemplate = getTemplateByQuery(chipRemoveSelector) || templates.remove,
          ...
          staticChips = userTemplate.find('md-chip'); // Warn of malformed template. See #2545
      ...
      return function postLink(scope, element, attrs, controllers) {
        ...
        if (attr.ngModel) {
          mdChipsCtrl.configureNgModel(element.controller('ngModel')); // If an `md-transform-chip` attribute was set, tell the controller to use the expression
          // before appending chips.

          if (attrs.mdTransformChip) mdChipsCtrl.useTransformChipExpression(); // If an `md-on-append` attribute was set, tell the controller to use the expression
          // when appending chips.
          //
          // DEPRECATED: Will remove in official 1.0 release

          if (attrs.mdOnAppend) mdChipsCtrl.useOnAppendExpression(); // If an `md-on-add` attribute was set, tell the controller to use the expression
          // when adding chips.

          if (attrs.mdOnAdd) mdChipsCtrl.useOnAddExpression(); // If an `md-on-remove` attribute was set, tell the controller to use the expression
          // when removing chips.

          if (attrs.mdOnRemove) mdChipsCtrl.useOnRemoveExpression(); // If an `md-on-select` attribute was set, tell the controller to use the expression
          // when selecting chips.

          if (attrs.mdOnSelect) mdChipsCtrl.useOnSelectExpression(); // The md-autocomplete and input elements won't be compiled until after this directive
