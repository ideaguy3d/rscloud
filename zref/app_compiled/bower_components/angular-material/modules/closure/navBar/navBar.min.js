...
function MdNavBarController(t, e, r, n) {
  this._$timeout = r, this._$scope = e, this._$mdConstant = n, this.mdSelectedNavItem, this.navBarAriaLabel, this._navBarEl = t[0], this._inkbar;
  ...
  return {
    ...
    template: function (t, e) {
      var r,
          ...
          i = e.mdNavSref,
          s = e.srefOpts;
      if ((o ? 1 : 0) + (l ? 1 : 0) + (i ? 1 : 0) > 1) throw Error("Must not specify more than one of the md-nav-click, md-nav-href, or md-nav-sref attributes per nav-item directive.");
      return o ? r = 'ng-click="ctrl.mdNavClick()"' : l ? r = 'ng-href="{{ctrl.mdNavHref}}"' : i && (r = 'ui-sref="{{ctrl.mdNavSref}}"'), n = s ? 'ui-sref-opts="{{ctrl.srefOpts}}" ' : "", r && (a = '<md-button class="_md-nav-button md-accent" ng-class="ctrl.getNgClassMap()" ng-blur="ctrl.setFocused(false)" tabindex="-1" ' + n + r + '><span ng-transclude class="_md-nav-button-text"></span></md-button>'), '<li class="md-nav-item" role="option" aria-selected="{{ctrl.isSelected()}}">' + (a || "") + "</li>";
    },
    scope: {
      mdNavClick: "&?",
      mdNavHref: "@?",
    ...
    link: function (r, n, a, o) {
      e(function () {
        var e = o[0],
            a = o[1],
            l = angular.element(n[0].querySelector("._md-nav-button"));
        e.name || (e.name = angular.element(n[0].querySelector("._md-nav-button-text")).text().trim()), l.on("click", function () {
          a.mdSelectedNavItem = e.name, r.$apply();
        }), t.expectWithText(n, "aria-label");
      });
    }
  };
}
...
goog.provide("ngmaterial.components.navBar"), goog.require("ngmaterial.core"), MdNavBarController.$inject = ["$element", "$scope", "$timeout", "$mdConstant"], MdNavItem.$inject = ["$mdAria", "$$rAF"], MdNavItemController.$inject = ["$element"], MdNavBar.$inject = ["$mdAria", "$mdTheming"], angular.module("material.components.navBar", ["material.core"]).controller("MdNavBarController", MdNavBarController).directive("mdNavBar", MdNavBar).controller("MdNavItemController", MdNavItemController).directive("mdNavItem", MdNavItem), MdNavBarController.prototype._initTabs = function () {
  ...
  if (n) {
    var a = -1,
        ...
        i = this._getTabByName(e);

    i && (i.setSelected(!1), a = n.indexOf(i)), l && (l.setSelected(!0), o = n.indexOf(l)), this._$timeout(function () {
      r._updateInkBarStyles(l, o, a);
    });
  }
}, MdNavBarController.prototype._updateInkBarStyles = function (t, e, r) {
  if (this._inkbar.toggleClass("_md-left", e < r).toggleClass("_md-right", e > r), this._inkbar.css({
    display: e < 0 ? "none" : ""
  }), t) {
    var n = t.getButtonEl(),
        a = n.offsetLeft;
  ...
  var t = Array.prototype.slice.call(this._navBarEl.querySelectorAll(".md-nav-item")).map(function (t) {
    return angular.element(t).controller("mdNavItem");
  });
  return t.indexOf(void 0) ? t : null;
  ...
  for (var e = this._getTabs(), r = 0; r < e.length; r++) if (t(e[r])) return e[r];
  ...
  if (n) {
    ...
    switch (t.keyCode) {
      case e.UP_ARROW:
      case e.LEFT_ARROW:
        a > 0 && this._moveFocus(n, r[a - 1]);
        break;

      case e.DOWN_ARROW:
      case e.RIGHT_ARROW:
        a < r.length - 1 && this._moveFocus(n, r[a + 1]);
        break;
