...
function SelectDirective($mdSelect, $mdUtil, $mdConstant, $mdTheming, $mdAria, $parse, $sce, $injector) {
  ...
  function compile(element, attr) {
    ...
    if (attr.mdOnOpen) {
      ...
      element.find('md-content').prepend(angular.element('<div>' + ' <md-progress-circular md-mode="indeterminate" ng-if="$$loadingAsyncDone === false" md-diameter="25px"></md-progress-circular>' + '</div>')); // Hide list [of item options] while loading async
    ...
    var isMultiple = $mdUtil.parseAttributeBoolean(attr.multiple); // Use everything that's left inside element.contents() as the contents of the menu
    ...
    return function postLink(scope, element, attr, ctrls) {
      ...
      var formCtrl = ctrls[3]; // grab a reference to the select menu value label
      ...
      mdSelectCtrl.setLabelText = function (text) {
        mdSelectCtrl.setIsPlaceholder(!text); // Whether the select label has been given via user content rather than the internal
...
function SelectMenuDirective($parse, $mdUtil, $mdConstant, $mdTheming) {
  ...
  function SelectMenuController($scope, $attrs, $element) {
    var self = this;
    self.isMultiple = angular.isDefined($attrs.multiple); // selected is an object with keys matching all of the selected options' hashed values
    ...
    self.setMultiple = function (isMultiple) {
      ...
      if (self.isMultiple) {
        ...
        ngModel.$render = renderMultiple; // watchCollection on the model because by default ngModel only watches the model's
    ...
    self.optNodeForKeyboardSearch = function (e) {
      ...
      clearSearchTimeout = setTimeout(function () {
        ...
        optText = undefined;
        optNodes = undefined;
      }, CLEAR_SEARCH_AFTER); // Support 1-9 on numpad
    ...
    self.init = function (ngModel, binding) {
      ...
      self.modelBinding = binding; // Setup a more robust version of isEmpty to ensure value is a valid option
      ...
      if (selectedOptionEls.length) {
        ...
        if (mode == 'html') {
          ...
          mapFn = function (el) {
            ...
            var html = el.innerHTML; // Remove the ripple container from the selected option, copying it would cause a CSP violation.
    ...
    self.addOption = function (hashKey, optionCtrl) {
      ...
      self.options[hashKey] = optionCtrl; // If this option's value was already in our ngModel, go ahead and select it.

      if (angular.isDefined(self.selected[hashKey])) {
        self.select(hashKey, optionCtrl.value); // When the current $modelValue of the ngModel Controller is using the same hash as
    ...
    self.removeOption = function (hashKey) {
      delete self.options[hashKey]; // Don't deselect an option when it's removed - the user's ngModel should be allowed
...
function SelectProvider($$interimElementProvider) {
  ...
  function selectDefaultOptions($mdSelect, $mdConstant, $mdUtil, $window, $q, $$rAF, $animateCss, $animate, $document) {
    ...
    function onRemove(scope, element, opts) {
      ...
      opts.hideBackdrop(); // For navigation $destroy events, do a quick, non-animated removal,
    ...
    function onShow(scope, element, opts) {
      ...
      function activateResizing() {
        ...
        window.on('orientationchange', debouncedOnResize); // Publish deactivation closure...
      ...
      function activateInteraction() {
        ...
        element.addClass('md-clickable'); // Close on backdrop click

        opts.backdrop && opts.backdrop.on('click', onBackdropClick); // Escape to close
    ...
    function calculateMenuPositions(scope, element, opts) {
      ...
      if (!loading) {
        ...
        if (selectedNode) {
          ...
          centeredNode = optgroupNodes[0]; // Otherwise - if we are not loading async - center around the first optionNode
      ...
      opts.focusedNode = focusedNode; // Get the selectMenuRect *after* max-width is possibly set above
  ...
  function calculateScrollable(element, contentNode) {
    ...
    try {
      var oldDisplay = element[0].style.display; // Set the element's display to block so that this calculation is correct

      element[0].style.display = 'block';
      isScrollable = contentNode.scrollHeight > contentNode.offsetHeight; // Reset it back afterwards
