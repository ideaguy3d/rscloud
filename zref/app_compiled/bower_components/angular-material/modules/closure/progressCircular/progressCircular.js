...
function MdProgressCircularDirective($window, $mdProgressCircular, $mdTheming, $mdUtil, $interval, $log) {
  ...
  function MdProgressCircularLink(scope, element, attrs) {
    ...
    var interval;
    $mdTheming(element);
    element.toggleClass(DISABLED_CLASS, attrs.hasOwnProperty('disabled')); // If the mode is indeterminate, it doesn't need to
    ...
    scope.$watchGroup(['value', 'mdMode', function () {
      var isDisabled = node.disabled; // Sometimes the browser doesn't return a boolean, in
    ...
    scope.$watch('mdDiameter', function (newValue) {
      ...
      svg[0].setAttribute('viewBox', '0 0 ' + diameter + ' ' + diameter); // Usually viewBox sets the dimensions for the SVG, however that doesn't
    ...
    function renderCircle(animateFrom, animateTo, easing, duration, iterationCount, maxValue) {
      ...
      var dashLimit = maxValue || 100; // No need to animate it if the values are the same
      ...
      if (animateTo === animateFrom) {
        ...
        lastDrawFrame = rAF(function animation() {
          ...
          renderFrame(ease(currentTime, animateFrom, changeInValue, animationDuration)); // Do not allow overlapping animations
    ...
    function animateIndeterminate() {
      renderCircle(startIndeterminate, endIndeterminate, $mdProgressCircular.easeFnIndeterminate, $mdProgressCircular.durationIndeterminate, iterationCount, 75); // The %4 technically isn't necessary, but it keeps the rotation
