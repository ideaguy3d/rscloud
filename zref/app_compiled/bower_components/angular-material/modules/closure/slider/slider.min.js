  ...
  function c(n, u, c, m) {
    ...
    function W(e) {
      if (!U()) {
        ...
        var t = F(P(ne ? e.pointer.y : e.pointer.x)),
            a = M(A(t));
        n.$apply(function () {
          k(a), R(z(a));
        });
    ...
    var B = m[0] || {
      ...
      return u[0].hasAttribute("disabled");
        ...
        ie = angular.isDefined(c.mdInvert);
    ...
    var re = angular.noop;
    c.ngDisabled && (re = n.$parent.$watch(c.ngDisabled, $)), o.register(Z, "drag", {
      horizontal: !ne
    }), n.mouseActive = !1, Z.on("keydown", x).on("mousedown", D).on("focus", S).on("blur", V).on("$md.pressdown", W).on("$md.pressup", N).on("$md.dragstart", T).on("$md.drag", _).on("$md.dragend", I), setTimeout(f, 0);
    var oe = e.throttle(f);
