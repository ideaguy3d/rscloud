...
function mdIconDirective($mdIcon, $mdTheming, $mdAria, $sce) {
  ...
  function postLink(scope, element, attr) {
    ...
    attr.$observe('mdFontSet', fontIconChanged); // Keep track of the content of the svg src so we can compare against it later to see if the
    // attribute is static (and thus safe).

    var originalSvgSrc = element[0].getAttribute(attr.$attr.mdSvgSrc); // If using a font-icon, then the textual name of the icon itself
...
function MdIconService(config, $templateRequest, $q, $log, $mdUtil, $sce) {
  ...
  getIcon.fontSet = findRegisteredFontSet; // Publish service...
  ...
  function getIcon(id) {
    id = id || ''; // If the "id" provided is not a string, the only other valid value is a $sce trust wrapper
  ...
  function transformClone(cacheElement) {
    ...
    var cacheSuffix = '_cache' + $mdUtil.nextUid(); // We need to modify for each cached icon the id attributes.
