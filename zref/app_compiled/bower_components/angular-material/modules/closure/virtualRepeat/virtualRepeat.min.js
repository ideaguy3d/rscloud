...
function VirtualRepeatContainerController(t, e, i, r, s, n, o, a, l) {
  ...
  }))) : this.topIndex = 0, this.scroller = a[0].querySelector(".md-virtual-repeat-scroller"), this.sizer = this.scroller.querySelector(".md-virtual-repeat-sizer"), this.offsetter = this.scroller.querySelector(".md-virtual-repeat-offsetter");
  var h = angular.bind(this, this.updateSize);
  t(angular.bind(this, function () {
    h();
    var t = e.debounce(h, 10, null, !1),
        i = angular.element(n);
    this.size || t(), i.on("resize", t), o.$on("$destroy", function () {
      i.off("resize", t);
  ...
  return {
    ...
    compile: function (e, i) {
      var r = i.mdVirtualRepeat,
          ...
          n = s[1],
          o = t(s[2]),
          a = i.mdExtraName && t(i.mdExtraName);
...
var NUM_EXTRA = 3;
VirtualRepeatContainerController.prototype.register = function (t) {
  ...
  if (this.sizer.innerHTML = "", t < this.maxElementPixels) this.sizer.style[e] = t + "px";else {
    ...
    var r = Math.floor(t / this.maxElementPixels),
        s = document.createElement("div");
    s.style[e] = this.maxElementPixels + "px", s.style[i] = "1px";
  ...
  if (!(e === this.scrollOffset || e > this.scrollSize - this.size)) {
    ...
    if (i) {
      var r = Math.max(0, Math.floor(e / i) - NUM_EXTRA),
          s = (this.isHorizontal() ? "translateX(" : "translateY(") + (!this.isHorizontal() || t ? r * i : -(r * i)) + "px)";

      if (this.scrollOffset = e, this.offsetter.style.webkitTransform = s, this.offsetter.style.transform = s, this.bindTopIndex) {
        var n = Math.floor(e / i);
        n !== this.topIndex && n < this.repeater.getItemCount() && (this.topIndex = n, this.bindTopIndex.assign(this.$scope, n), this.$rootScope.$$phase || this.$scope.$digest());
      }
  ...
  if (this.items && i < this.items.length && 0 !== this.container.getScrollOffset()) {
    ...
    this.container.resetScroll(), this.container.scrollTo(s);
  }

  i !== this.itemsLength && (r = !0, this.itemsLength = i), this.items = t, (t !== e || r) && this.updateIndexes_(), this.parentNode = this.$element[0].parentNode, r && this.container.setScrollSize(i * this.itemSize);
  var n,
      o = !1;
  this.isFirstRender && (o = !0, this.isFirstRender = !1, n = this.$attrs.mdStartIndex ? this.$scope.$eval(this.$attrs.mdStartIndex) : this.container.topIndex, this.container.scrollToIndex(n)), Object.keys(this.blocks).forEach(function (t) {
    var e = parseInt(t, 10);
  ...
  for (a = this.newStartIndex; a < this.newEndIndex && null == this.blocks[a]; a++) l = this.getBlock_(a), this.updateBlock_(l, a), h.push(l);

  for (; null != this.blocks[a]; a++) this.updateBlock_(this.blocks[a], a);

  for (var c = a - 1; a < this.newEndIndex; a++) l = this.getBlock_(a), this.updateBlock_(l, a), p.push(l);

  h.length && this.parentNode.insertBefore(this.domFragmentFromBlocks_(h), this.$element[0].nextSibling), p.length && this.parentNode.insertBefore(this.domFragmentFromBlocks_(p), this.blocks[c] && this.blocks[c].element[0].nextSibling), o && this.container.scrollToIndex(n), this.$browser.$$checkUrlChange = this.browserCheckUrlChange, this.startIndex = this.newStartIndex, this.endIndex = this.newEndIndex, this.isVirtualRepeatUpdating_ = !1;
  ...
  return this.transclude(angular.bind(this, function (i, r) {
    e = {
      ...
      "new": !0,
      scope: r
    }, this.updateScope_(r, t), this.parentNode.appendChild(i[0]);
  })), e;
