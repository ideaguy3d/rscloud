...
function MdTooltipDirective($timeout, $window, $$rAF, $document, $interpolate, $mdUtil, $mdPanel, $$mdTooltipRegistry) {
  ...
  function linkFunc(scope, element, attr) {
    ...
    var origin,
        ...
        autohide,
        showTimeout,
        elementFocusedOnWindowBlur = null; // Set defaults

    setDefaults(); // Set parent aria-label.

    addAriaLabel(); // Remove the element from its current DOM position.
    ...
    function addAriaLabel(labelText) {
      ...
      var interpolatedText = labelText || $interpolate(element.text().trim())(scope.$parent); // Only add the `aria-label` to the parent if there isn't already one, if there isn't an
      ...
      if (!parent.attr('aria-label') && !parent.attr('aria-labelledby') || parent.attr('md-labeled-by-tooltip')) {
        parent.attr('aria-label', interpolatedText); // Set the `md-labeled-by-tooltip` attribute if it has not already been set.
    ...
    function updatePosition() {
      setDefaults(); // If the panel has already been created, remove the current origin
      ...
      origin = 'md-origin-' + scope.mdDirection; // Create the position of the panel based off of the mdDirection.

      position = TOOLTIP_DIRECTIONS[scope.mdDirection]; // Using the newly created position object, use the MdPanel
      // panelPosition API to build the panel's position.

      panelPosition = $mdPanel.newPanelPosition().relativeTo(parent).addPanelPosition(position.x, position.y); // If the panel has already been created, add the new origin class to
    ...
    function bindEvents() {
      ...
      scope.$on('$destroy', onDestroy); // To avoid 'synthetic clicks', we listen to mousedown instead of
      // 'click'.
      ...
      function enterEventHandler($event) {
        ...
        if ($event.type === 'focus' && elementFocusedOnWindowBlur) {
          ...
          setVisible(true); // If the user is on a touch device, we should bind the tap away
      ...
      function onDestroy() {
        ...
        parent.off(ENTER_EVENTS, enterEventHandler).off(LEAVE_EVENTS, leaveEventHandler).off('mousedown', mousedownEventHandler); // Trigger the handler in case any of the tooltips are
        // still visible.
    ...
    function configureWatchers() {
      ...
      scope.$watch('mdDirection', updatePosition); // Clean up if the element or parent was removed via jqLite's .remove.
      // A couple of notes:
