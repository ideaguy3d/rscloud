...
function MdAutocompleteCtrl(e, t, n, o, i, l, r, a, c, u, d, m) {
  function s() {
    n.initOptionalProperties(e, c, {
      searchText: "",
      selectedItem: null,
      clearButton: !1
    }), i(t), $(), n.nextTick(function () {
  ...
  function h() {
    ...
    function i() {
      var e = Ce.scrollContainer.getBoundingClientRect(),
          t = {};
      e.right > d.right - MENU_PADDING && (t.left = a.right - e.width + "px"), Ce.$.scrollContainer.css(t);
    }

    if (!Ce) return n.nextTick(h, !1, e);
    var l,
        r = (e.dropdownItems || MAX_ITEMS) * ITEM_HEIGHT,
        a = Ce.wrap.getBoundingClientRect(),
        u = Ce.snap.getBoundingClientRect(),
        d = Ce.root.getBoundingClientRect(),
        m = u.bottom - d.top,
        s = d.bottom - u.top,
        p = a.left - d.left,
        g = a.width,
        f = o(),
        $ = e.dropdownPosition;
    if ($ || ($ = m > s && d.height - m - MENU_PADDING < r ? "top" : "bottom"), c.mdFloatingLabel && (p += INPUT_PADDING, g -= 2 * INPUT_PADDING), l = {
      left: p + "px",
      minWidth: g + "px",
      maxWidth: Math.max(a.right - d.left, d.right - a.left) - MENU_PADDING + "px"
    }, "top" === $) l.top = "auto", l.bottom = s + "px", l.maxHeight = Math.min(r, a.top - d.top - MENU_PADDING) + "px";else {
      var x = d.bottom - a.bottom - MENU_PADDING + n.getViewportTop();
      l.top = m - f + "px", l.bottom = "auto", l.maxHeight = Math.min(r, x) + "px";
    }
    Ce.$.scrollContainer.css(l), n.nextTick(i, !1);
  }

  function g() {
    Ce.$.root.length && (i(Ce.$.scrollContainer), Ce.$.scrollContainer.detach(), Ce.$.root.append(Ce.$.scrollContainer), r.pin && r.pin(Ce.$.scrollContainer, a));
  }
  ...
  function $() {
    ...
    }), e.$watch("searchText", t ? n.debounce(L, t) : L), e.$watch("selectedItem", y), angular.element(l).on("resize", De), e.$on("$destroy", x);
  }

  function x() {
    if (fe.hidden || n.enableScrolling(), angular.element(l).off("resize", De), Ce) {
  ...
  function b() {
    ...
    for (e = t; e.length && (n = e.attr("md-autocomplete-snap"), !angular.isDefined(n)); e = e.parent());

    if (e.length) return {
      snap: e[0],
      wrap: "width" === n.toLowerCase() ? e[0] : t.find("md-autocomplete-wrap")[0]
    };
    var o = t.find("md-autocomplete-wrap")[0];
    return {
      snap: o,
      wrap: o
    };
  ...
  function M(e, t) {
    !e && t ? (h(), re(!0, ye.Count | ye.Selected), Ce && (n.disableScrollAround(Ce.ul), we = T(angular.element(Ce.wrap)))) : e && !t && (n.enableScrolling(), we && (we(), we = null));
  }
  ...
  function I() {
    Me || fe.hidden || Ce.input.focus(), be = !1, fe.hidden = V();
  }
  ...
  function y(t, n) {
    p(), t ? B(t).then(function (o) {
      ...
      angular.isString(e.searchText) && t.toString().toLowerCase() === e.searchText.toLowerCase() && (e.searchText = "");
    }), t !== n && E();
  }
  ...
  function P(e) {
    Me = !1, be || (fe.hidden = V(), ge("ngBlur", {
      $event: e
    }));
  }

  function k(e) {
    e && (be = !1, Me = !1), Ce.input.blur();
  }

  function O(e) {
    Me = !0, j() && Z() && se(), fe.hidden = V(), ge("ngFocus", {
      $event: e
    });
  }
  ...
  function _(t) {
    switch (t.keyCode) {
      case o.KEY_CODE.DOWN_ARROW:
        if (fe.loading) return;
        t.stopPropagation(), t.preventDefault(), fe.index = Math.min(fe.index + 1, fe.matches.length - 1), ce(), re(!1, ye.Selected);
        break;

      case o.KEY_CODE.UP_ARROW:
        if (fe.loading) return;
        t.stopPropagation(), t.preventDefault(), fe.index = fe.index < 0 ? fe.matches.length - 1 : Math.max(0, fe.index - 1), ce(), re(!1, ye.Selected);
        break;

      case o.KEY_CODE.TAB:
        if (I(), fe.hidden || fe.loading || fe.index < 0 || fe.matches.length < 1) return;
        te(fe.index);
        break;

      case o.KEY_CODE.ENTER:
        if (fe.hidden || fe.loading || fe.index < 0 || fe.matches.length < 1) return;
        if (X()) return;
        t.stopPropagation(), t.preventDefault(), te(fe.index);
        break;

      case o.KEY_CODE.ESCAPE:
        if (t.preventDefault(), !z()) return;
        t.stopPropagation(), oe(), e.searchText && W("clear") && ie(), fe.hidden = !0, W("blur") && k(!0);
    }
  }
  ...
  function F(e) {
    if (e) {
      var t = {};
      return fe.itemName && (t[fe.itemName] = e), t;
    }
  }
  ...
  function re(e, t) {
    var n = e ? "polite" : "assertive",
        o = [];
    t & ye.Selected && fe.index !== -1 && o.push(Q()), t & ye.Count && o.push(u.resolve(ae())), u.all(o).then(function (e) {
      m.announce(e.join(" "), n);
    });
  }

  function ae() {
    switch (fe.matches.length) {
      case 0:
  ...
  function ce() {
    if (Ce.li[0]) {
      var e = Ce.li[0].offsetHeight,
          t = e * fe.index,
          n = t + e,
          o = Ce.scroller.clientHeight,
          i = Ce.scroller.scrollTop;
      t < i ? de(t) : n > i + o && de(n - o);
    }
  }
  ...
  function me() {
    var e = (fe.scope.searchText || "").length;
    return fe.hasNotFound && !Y() && (!fe.loading || ue()) && e >= q() && (Me || be) && !X();
  }

  function se() {
    var t = e.searchText || "",
        n = t.toLowerCase();
    !e.noCache && ve[n] ? pe(ve[n]) : le(t), fe.hidden = V();
  }
  ...
  function he() {
    ...
    1 === n.length && B(o).then(function (n) {
      var o = t == n;
      e.matchInsensitive && !o && (o = t.toLowerCase() == n.toLowerCase()), o && te(0);
    });
  }
  ...
  var fe = this,
      ...
      Ie = null,
      De = n.debounce(C);
  ee("hidden", M, !0), fe.scope = e, fe.parent = e.$parent, fe.itemName = $e[0], fe.matches = [], fe.loading = !1, fe.hidden = !0, fe.index = null, fe.id = n.nextUid(), fe.isDisabled = null, fe.isRequired = null, fe.isReadonly = null, fe.hasNotFound = !1, fe.keydown = _, fe.blur = P, fe.focus = O, fe.clear = ne, fe.select = te, fe.listEnter = w, fe.listLeave = I, fe.mouseUp = D, fe.getCurrentDisplayValue = Q, fe.registerSelectedItemWatcher = S, fe.unregisterSelectedItemWatcher = H, fe.notFoundVisible = me, fe.loadingIsVisible = J, fe.positionDropdown = h;
  var ye = {
    Count: 1,
    Selected: 2
  };
  ...
  function n(e, n, o) {
    return function (e, n, i) {
      ...
      var a = e.$mdAutocompleteCtrl,
          c = a.parent.$new(),
          u = a.itemName;
      l("$index", "$index"), l("item", u), r(), o(c, function (e) {
        n.after(e);
      });
    };
  }
...
var ITEM_HEIGHT = 48,
    ...
    MENU_PADDING = 8,
    INPUT_PADDING = 2;
MdAutocomplete.$inject = ["$$mdSvgRegistry"], angular.module("material.components.autocomplete").directive("mdAutocomplete", MdAutocomplete), MdAutocompleteItemScopeDirective.$inject = ["$compile", "$mdUtil"], angular.module("material.components.autocomplete").directive("mdAutocompleteParentScope", MdAutocompleteItemScopeDirective), MdHighlightCtrl.$inject = ["$scope", "$element", "$attrs"], angular.module("material.components.autocomplete").controller("MdHighlightCtrl", MdHighlightCtrl), MdHighlightCtrl.prototype.init = function (e, t) {
  ...
  this.$element.empty(), t.forEach(function (e) {
    if (e.isMatch) {
      var t = angular.element('<span class="highlight">').text(e.text);
