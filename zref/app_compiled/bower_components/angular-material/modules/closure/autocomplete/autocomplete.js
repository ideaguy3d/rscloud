...
function MdAutocompleteCtrl($scope, $element, $mdUtil, $mdConstant, $mdTheming, $window, $animate, $rootElement, $attrs, $q, $log, $mdLiveAnnouncer) {
  ...
  var ctrl = this,
      ...
      inputModelCtrl = null,
      debouncedOnResize = $mdUtil.debounce(onWindowResize); // Public Exported Variables with handlers

  defineProperty('hidden', handleHiddenChange, true); // Public Exported Variables
  ...
  ctrl.hasNotFound = false; // Public Exported Methods
  ...
  return init(); //-- initialization methods
  ...
  function init() {
    ...
    $mdUtil.nextTick(function () {
      gatherElements();
      moveDropdown(); // Forward all focus events to the input element when autofocus is enabled
  ...
  function positionDropdown() {
    ...
    var hrect = elements.wrap.getBoundingClientRect(),
        ...
        styles; // Automatically determine dropdown placement based on available space in viewport.
    ...
    function getVerticalOffset() {
      ...
      if (inputContainer.length) {
        ...
        offset -= input.prop('offsetHeight'); // add in the height left up top for the floating label text
  ...
  function handleHiddenChange(hidden, oldHidden) {
    if (!hidden && oldHidden) {
      positionDropdown(); // Report in polite mode, because the screenreader should finish the default description of
      // the input. element.
  ...
  function handleSearchText(searchText, previousSearchText) {
    ctrl.index = getDefaultIndex(); // do nothing on init
    ...
    getDisplayValue($scope.selectedItem).then(function (val) {
      ...
      if (searchText !== val) {
        $scope.selectedItem = null; // trigger change event if available
  ...
  function clearSearchText() {
    ...
    setLoading(true);
    $scope.searchText = ''; // Normally, triggering the change / input event is unnecessary, because the browser detects it properly.
    ...
    elements.input.dispatchEvent(eventObj); // For some reason, firing the above event resets the value of $scope.searchText if
  ...
  function fetchResults(searchText) {
    ...
    function onResultsRetrieved(matches) {
      cache[term] = matches; // Just cache the results if the request is now outdated.
  ...
  function handleQuery() {
    ...
    var term = searchText.toLowerCase(); // If caching is enabled and the current searchText is stored in the cache
  ...
  function handleResults(results) {
    ...
    ctrl.hidden = shouldHide(); // If loading is in progress, then we'll end the progress. This is needed for example,
  ...
  return {
    ...
    compile: function (tElement, tAttrs) {
      ...
      return function (scope, element, attrs, ctrl) {
        ...
        ctrl.hasNotFound = !!element.attr('md-has-not-found'); // By default the inset autocomplete should show the clear button when not explicitly overwritten.
    ...
    template: function (element, attr) {
      var noItemsTemplate = getNoItemsTemplate(),
          ...
          tabindex = attr.tabindex; // Set our attribute for the link function above which runs later.
      ...
      if (noItemsTemplate) element.attr('md-has-not-found', true); // Always set our tabindex of the autocomplete directive to -1, because our input
...
function MdAutocompleteItemScopeDirective($compile, $mdUtil) {
  ...
  function compile(tElement, tAttr, transclude) {
    return function postLink(scope, element, attr) {
      ...
      var itemName = ctrl.itemName; // Watch for changes to our scope's variables and copy them to the new scope

      watchVariable('$index', '$index');
      watchVariable('item', itemName); // Ensure that $digest calls on our scope trigger $digest on newScope.

      connectScopes(); // Link the element against newScope.
...
function MdHighlightCtrl($scope, $element, $attrs) {
  ...
  this.$attrs = $attrs; // Cache the Regex to avoid rebuilding each time.
