...
(function (global, factory) {
...
})(void 0, function (exports, ng_from_import) {
  'use strict';
  /**
  ...
  var RegisteredHook = function () {
    ...
    RegisteredHook.prototype.matches = function (treeChanges) {
      var matches = this._getMatchingNodes(treeChanges); // Check if all the criteria matched the TreeChanges object
  ...
  function makeEvent(registry, transitionService, eventType) {
    ...
    var hooks = _registeredHooks[eventType.name] = []; // Create hook registration function on the IHookRegistry for the event
  ...
  var HookBuilder = function () {
    ...
    HookBuilder.prototype.buildHooks = function (hookType) {
      ...
      var makeTransitionHooks = function makeTransitionHooks(hook) {
        ...
        var matches = hook.matches(treeChanges); // Select the PathNode[] that will be used as TransitionHook context objects
    ...
    HookBuilder.prototype.getMatchingHooks = function (hookType, treeChanges) {
      var isCreate = hookType.hookPhase === exports.TransitionHookPhase.CREATE; // Instance and Global hook registries
  ...
  var Param = function () {
    function Param(id, type, config, location, urlMatcherFactory) {
      ...
      var inherit$$1 = isDefined(config.inherit) ? !!config.inherit : !!type.inherit; // array config: param name (param[]) overrides default settings.  explicit config overrides param name.
  ...
  var Resolvable = function () {
    ...
    Resolvable.prototype.resolve = function (resolveContext, trans) {
      ...
      var maybeWaitForRx = this.getPolicy(state).async === "RXWAIT" ? waitForRx : identity; // After the final value has been resolved, update the state of the Resolvable
  ...
  var beforeAfterSubstr = function beforeAfterSubstr(_char) {
    return function (str) {
      if (!str) return ["", ""];
      var idx = str.indexOf(_char);
      if (idx === -1) return [str, ""];
  ...
  var getUrlBuilder = function getUrlBuilder($urlMatcherFactoryProvider, root) {
    return function urlBuilder(state) {
      var stateDec = state; // For future states, i.e., states whose name ends with `.**`,
  ...
  var StateRegistry = function () {
    ...
    StateRegistry.prototype._deregisterTree = function (state) {
      ...
      deregistered.forEach(function (state) {
        var $ur = _this._router.urlRouter; // Remove URL rule

        $ur.rules().filter(propEq("state", state)).forEach($ur.removeRule.bind($ur)); // Remove state from registry
  ...
  var UrlMatcher = function () {
    ...
    function UrlMatcher(pattern$$1, paramTypes, paramFactory, config) {
      ...
      segment = pattern$$1.substring(last); // Find any search parameter names and remove them from the last segment
  ...
  var UrlRouter = function () {
    ...
    UrlRouter.prototype.match = function (url) {
      ...
      if (this._otherwiseFn) rules.push(this._otherwiseFn); // Checks a single rule. Returns { rule: rule, match: match, weight: weight } if it matched, or undefined
  ...
  var ViewService = function () {
    ...
    ViewService.prototype.sync = function () {
      ...
      var configureUIView = function configureUIView(_a) {
        var uiView = _a[0],
            viewConfig = _a[1]; // If a parent ui-view is reconfigured, it could destroy child ui-views.
  ...
  var UrlService = function () {
    ...
    function UrlService(router, lateBind) {
      ...
      this.router = router;
      this.rules = {};
      this.config = {}; // proxy function calls from UrlService to the LocationService/LocationConfig
  ...
  var registerOnExitHook = function registerOnExitHook(transitionService) {
    return transitionService.onExit({
      exiting: function exiting(state) {
        return !!state.onExit;
      }
    }, onExitHook);
  };
  ...
  var registerOnRetainHook = function registerOnRetainHook(transitionService) {
    return transitionService.onRetain({
      retained: function retained(state) {
  ...
  var TransitionService = function () {
    ...
    TransitionService.prototype._registerCoreTransitionHooks = function () {
      ...
      fns.lazyResolve = registerLazyResolveState(this); // Wire up the View management hooks
  ...
  var beforeAfterSubstr$1 = function beforeAfterSubstr$1(_char2) {
    return function (str) {
      if (!str) return ["", ""];
      var idx = str.indexOf(_char2);
      if (idx === -1) return [str, ""];
      return [str.substr(0, idx), str.substr(idx + 1)];
    };
  };
    ...
    BaseLocationServices.prototype.url = function (url, replace) {
      ...
      if (isDefined(url) && url !== this._get()) {
        ...
        if (this.fireAfterUpdate) {
          var evt_1 = extend(new Event("locationchange"), {
            url: url
          });
  ...
  var Ng1LocationServices = function () {
    ...
    Ng1LocationServices.prototype._runtimeServices = function ($rootScope, $location, $sniffer, $browser) {
      ...
      var _browser = val($browser); // Bind these LocationService functions to $location


      createProxyFunctions(_loc, this, _loc, ["replace", "path", "search", "hash"]); // Bind these LocationConfig functions to $location

      createProxyFunctions(_loc, this, _loc, ['port', 'protocol', 'host']); // Bind these LocationConfig functions to $browser
  ...
  function runBlock($injector$$1, $q$$1, $uiRouter) {
    ...
    services.$q = $q$$1; // The $injector is now available.
    ...
    if (isFunction(controllerInstance.uiOnParamsChanged)) {
      ...
      var paramsUpdated = function paramsUpdated($transition$) {
        ...
        var fromSchema = $transition$.treeChanges().from.map(function (node) {
          return node.paramSchema;
        }).reduce(unnestR, []); // Find the to params that have different values than the from params
