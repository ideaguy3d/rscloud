...
(function (global, factory) {
  (typeof exports === "undefined" ? "undefined" : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? factory(exports, require('@uirouter/core'), require('angular')) : typeof define === 'function' && define.amd ? define(['exports', '@uirouter/core', 'angular'], factory) : factory(global['@uirouter/angularjs'] = global['@uirouter/angularjs'] || {}, global['@uirouter/core'], global.angular);
})(void 0, function (exports, _uirouter_core, ng_from_import) {
  'use strict';
  ...
  function ng1ViewsBuilder$1(state) {
    ...
    var views = {},
      ...
      "$default": _uirouter_core.pick(state, allViewKeys)
    };
    ...
    _uirouter_core.forEach(viewsObject, function (config, name) {
      ...
      if (_uirouter_core.isString(config)) config = {
        component: config
      }; // Make a shallow copy of the config object

      config = _uirouter_core.extend({}, config); // Do not allow a view to mix props for component-style view with props for template/controller-style view
  ...
  var Ng1ViewConfig$1 = function () {
    ...
    Ng1ViewConfig$$1.prototype.getController = function (context) {
      ...
      if (!_uirouter_core.isInjectable(provider)) return this.viewDecl.controller;
  ...
  var TemplateFactory = function () {
    ...
    TemplateFactory.prototype.fromConfig = function (config, params, context) {
      ...
      return _uirouter_core.isDefined(config.template) ? asTemplate(this.fromString(config.template, params)) : _uirouter_core.isDefined(config.templateUrl) ? asTemplate(this.fromUrl(config.templateUrl, params)) : _uirouter_core.isDefined(config.templateProvider) ? asTemplate(this.fromProvider(config.templateProvider, params, context)) : _uirouter_core.isDefined(config.component) ? asComponent(config.component) : _uirouter_core.isDefined(config.componentProvider) ? asComponent(this.fromComponentProvider(config.componentProvider, params, context)) : asTemplate(defaultTemplate);
    };
    /**
    ...
    TemplateFactory.prototype.fromUrl = function (url, params) {
      if (_uirouter_core.isFunction(url)) url = url(params);
    ...
    TemplateFactory.prototype.makeComponentTemplate = function (uiView, context, component, bindings) {
      ...
      var prefix = ng.version.minor >= 3 ? "::" : ""; // Convert to kebob name. Add x- prefix if the string starts with `x-` or `data-`
      ...
      var attributeTpl = function attributeTpl(input) {
        ...
        if (type === '&') {
          ...
          var fn = res && res.data;
          var args = fn && _uirouter_core.services.$injector.annotate(fn) || []; // account for array style injection, i.e., ['foo', function(foo) {}]
  ...
  var getBindings = function getBindings(def) {
    if (_uirouter_core.isObject(def.bindToController)) return scopeBindings(def.bindToController);
  ...
  var getStateHookBuilder = function getStateHookBuilder(hookName) {
    return function stateHookBuilder(state, parentFn) {
      ...
      function decoratedNg1Hook(trans, state) {
        ...
        var locals = _uirouter_core.extend(getLocals$1(resolveContext), {
          $state$: state,
          $transition$: trans
        });
  ...
  var Ng1LocationServices = function () {
    ...
    Ng1LocationServices.prototype._runtimeServices = function ($rootScope, $location, $sniffer, $browser) {
      ...
      this.$sniffer = $sniffer; // Bind $locationChangeSuccess to the listeners registered in LocationService.onChange
      ...
      var _browser = _uirouter_core.val($browser); // Bind these LocationService functions to $location


      _uirouter_core.createProxyFunctions(_loc, this, _loc, ["replace", "path", "search", "hash"]); // Bind these LocationConfig functions to $location


      _uirouter_core.createProxyFunctions(_loc, this, _loc, ['port', 'protocol', 'host']); // Bind these LocationConfig functions to $browser
  ...
  var UrlRouterProvider$1 = function () {
    ...
    UrlRouterProvider$$1.prototype.rule = function (ruleFn) {
      ...
      if (!_uirouter_core.isFunction(ruleFn)) throw new Error("'rule' must be a function");
  ...
  function $uiRouter($locationProvider) {
    ...
    router.stateProvider = new StateProvider$1(router.stateRegistry, router.stateService); // Apply ng1 specific StateBuilder code for `views`, `resolve`, and `onExit/Retain/Enter` properties
  ...
  function runBlock($injector, $q, $uiRouter) {
    ...
    _uirouter_core.services.$q = $q; // The $injector is now available.
    // Find any resolvables that had dependency annotation deferred

    $uiRouter.stateRegistry.get().map(function (x) {
      return x.$$state().resolvables;
    }).reduce(_uirouter_core.unnestR, []).filter(function (x) {
  ...
  var getLocals$1 = function getLocals$1(ctx) {
      ...
      compile: function compile(tElement, tAttrs, $transclude) {
        return function (scope, $element, attrs) {
          ...
          var activeUIView = {
            ...
            get creationContext() {
              var fromParentTagConfig = _uirouter_core.parse('$cfg.viewDecl.$context')(inherited); // Allow <ui-view name="foo"><ui-view name="bar"></ui-view></ui-view>
          ...
          function configUpdatedCallback(config) {
            if (config && !(config instanceof Ng1ViewConfig$1)) return;
    ...
    if (_uirouter_core.isFunction(controllerInstance.uiOnParamsChanged)) {
      ...
      var paramsUpdated = function paramsUpdated($transition$) {
        ...
        var fromSchema = $transition$.treeChanges().from.map(function (node) {
          return node.paramSchema;
        }).reduce(_uirouter_core.unnestR, []); // Find the to params that have different values than the from params
